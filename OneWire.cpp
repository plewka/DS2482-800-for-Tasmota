...
*/
OneWire::OneWire(TwoWire& i2c, uint8_t address): i2c(i2c)
{
	// Address is determined by two pins on the DS2482 AD1/AD0
	// Pass 0b00, 0b01, 0b10 or 0b11
	mAddress = DS2482_ADDRESS | address;
	mError = 0;
	//i2c.begin();
}
...
void OneWire::begin()
{
	i2c.beginTransmission(mAddress);
}
...
void printAddress(uint8_t deviceAddress[8])
{
  ...
  for (uint8_t i = 0; i < 8; i++)
  {
    ...
    dbgserial.print("0x");
    if (deviceAddress[i] < 16) dbgserial.print("0");
    dbgserial.print(deviceAddress[i], HEX);
    if (i < 7) dbgserial.print(", ");
...
void OneWire::scan()
{
  dbgserial.println("Checking for I2C devices...:");
  if (oneWire.checkPresence())
  {
    dbgserial.println("DS2482 present");

    oneWire.deviceReset();

    delay(5);

    for (int8_t room = 0; room <= 8; room++)
    {
      oneWire.setChannel(room);
      delay(5);

      dbgserial.println("\tChecking for 1-Wire devices...");
      if (oneWire.wireReset())
      {
        dbgserial.println("\tDevices present on 1-Wire bus");

        uint8_t currAddress[8];

        dbgserial.println("\t\tSearching 1-Wire bus...");

        while (oneWire.wireSearch(currAddress))
        {
          dbgserial.print("\t\t\tFound device: ");
        ...
        dbgserial.println("No devices on 1-Wire bus");
    }
  }
  else
    dbgserial.println("No DS2482 present");
}
